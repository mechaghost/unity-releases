...
- **After every release push, verify the site is up before reporting back to the user.** Wait for Railway to finish building (`railway service status --service unity-releases --json` until `SUCCESS`), then curl `/`, `/stats`, `/releases`, and `/api/health` and check for 200s. If any return 5xx, pull `railway logs --service unity-releases`, diagnose, fix, and push another release in the same session. Don't make the user catch breakage you can see yourself. Common failure modes: middleware bundle pulling in Node-only deps (Edge runtime can't load `pg`/`crypto`); schema changes that haven't been migrated to prod with `railway run npm run db:migrate`; server-only modules imported into client components.
