...
describe("release page filters", () => {
  test("defaults to the two current Unity 6 LTS lines", () => {
    expect(parseSelectedReleaseFilters(undefined)).toEqual(["6000.0-lts", "6000.3-lts"]);
  });

  test("maps the legacy combined LTS query to both split LTS filters", () => {
    expect(parseSelectedReleaseFilters("lts")).toEqual(["6000.0-lts", "6000.3-lts"]);
  });
  ...
  test("preserves split filters in pagination links", () => {
    ...
    expect(releasePageHref(2, ["6000.0-lts", "6000.3-lts"])).toBe("/releases?page=2");
  });
});
